...
本 change 不自动执行迁移。F15 已进入独立修复分支并完成本地验证，PR #144 已创建、待审查；后续按独立修复分支和 PR 交付，每个 PR 同步必要 README 文档，在规划分支分别记录审计、tasks 的实现验证状态和上游合并状态；只在测试证据证明后勾选。本 change 无凭据格式/证书/传输配置迁移，不要求批量更新 VM 配置。回滚插件时保留用户当前配置与有效凭据，不恢复过期密钥，不自动逆向撤销 TrueNAS 数据变更。任务产生的 wsx 临时目录/容器在测试结束清理，共享服务不动。

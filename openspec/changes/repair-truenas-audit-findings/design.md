...
本轮用户只授权审计与规划，不修改代码。未来按 [上游 PR 流程与拆分](upstream-pr-workflow.md) 逐组实施；五阶段作为任务分类，实际按 PR 依赖推进，不集中成五个大 PR；同一 Client 文件不并发分配多个写作者。
...
本轮不执行迁移或代码实施。未来按独立修复分支和 PR 交付，每个 PR 同步必要 README 文档，在规划分支分别记录审计、tasks 的实现验证状态和上游合并状态；只在测试证据证明后勾选。本 change 无凭据格式/证书/传输配置迁移，不要求批量更新 VM 配置。回滚插件时保留用户当前配置与有效凭据，不恢复过期密钥，不自动逆向撤销 TrueNAS 数据变更。任务产生的 wsx 临时目录/容器在测试结束清理，共享服务不动。

...
最初审计阶段只授权规划。用户随后授权首组 F15 修复、测试和本地提交，随后授权推送并手动创建 PR #144；第二组 F14/F26/F27 已获授权并完成本地修复、测试与提交，暂不推送或创建 PR；当前进度见 tasks 和实施记录。按 [上游 PR 流程与拆分](upstream-pr-workflow.md) 逐组实施；五阶段作为任务分类，实际按 PR 依赖推进，不集中成五个大 PR；同一 Client 文件不并发分配多个写作者。
...
参数、依赖、PVE 版本和资源检查必须先于 mv/rm/cp；未知参数立即退出。关键命令显式检查或配合正确初始化变量使用 shell 严格模式；首次安装无可选备份不能因单纯 set -e 被误伤。补丁先在副本验证可应用；失败报告阶段与恢复路径，不无条件重启 corosync/pve-cluster。按用户复核决定，本组成功安装后保留原有 corosync、pve-cluster、pvedaemon、pvestatd、pveproxy 重启列表和顺序；缩小范围只记录为脚本 TODO，待真实 PVE 验证后另行考虑。build.sh 与 deploy.sh 通过同目录 script-common.sh 复用错误输出、命令检查及 PVE 版本检测，其他阶段保持各自实现。
...
本 change 不自动执行迁移。F15 已进入独立修复分支并完成本地验证，PR #144 已于 2026-09-09 合并。第二组 F14/F26/F27 已完成本地提交与分组回归，未推送或创建 PR；后续按独立修复分支和 PR 交付，每个 PR 同步必要 README 文档，在规划分支分别记录审计、tasks 的实现验证状态和上游合并状态；只在测试证据证明后勾选。本 change 无凭据格式/证书/传输配置迁移，不要求批量更新 VM 配置。回滚插件时保留用户当前配置与有效凭据，不恢复过期密钥，不自动逆向撤销 TrueNAS 数据变更。任务产生的 wsx 临时目录/容器在测试结束清理，共享服务不动。
